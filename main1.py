...
stopword.extend(['take','call','get','first','second','third','need','needs','dont','want','give','go','given','turns','way','i\'m','seems','place','one','find','told','feeling','year','years','told','say','went','got','thing','take','taking','\'m','\'re','become','became','made','life'])
...
depressionModel = keras.models.load_model('deployables/depression.h5',custom_objects={"coeff_determination": coeff_determination})
anxietyModel = keras.models.load_model('deployables/anxiety.h5',custom_objects={"coeff_determination": coeff_determination})
stressModel = keras.models.load_model('deployables/stress.h5',custom_objects={"coeff_determination": coeff_determination})
...
def ret_results(text):
        ...
        for token in note:
            ...
            if token.pos_ in ['VERB','ADJ','NOUN']:
                ...
                if isNeg :
                    ...
                    sims=np.array([nlp2(outer(token.text.lower())).similarity(nlp2(lemmedWord)) for lemmedWord in lemmedWords]) 

                    word=antonyms[antonyms[0]==outer(token.text.lower())][1].values
                    ...
                    if len(word)==0:
                        if len(antonyms[antonyms[0]==lemmedWords[sims.argmax()]][1].values)==0:
                            ...
                            opword=antonyms[antonyms[0]==lemmedWords[sims.argmax()]][1].values[0]
                            sims=np.array([nlp2(opword).similarity(nlp2(lemmedWord)) for lemmedWord in lemmedWords])
                            ...
                            sims=np.array([nlp2(word[0]).similarity(nlp2(lemmedWord)) for lemmedWord in lemmedWords])
                ...
                if outer(token.text.lower()) in lemmedWords:
                        ...
                        vector_representation[lemmedWords.index(outer(token.text.lower()))]=0.99
                        selectedWords.add(token.text.lower())
                        continue
                if outer(token.text.lower()) in G.nodes():


                    neibs=[i for i in nx.neighbors(G,outer(token.text.lower())) if i in lemmedWords] 

                    if len(neibs)==0:
                        continue
                    sims=np.array([nlp2(outer(token.text.lower())).similarity(nlp2(neib)) for neib in neibs])
                    if sims.max()<0.5:
                    ...
                    if sims.max()==1.0:
                        ...
                        vector_representation[lemmedIndex[neibs[sims.argmax()]]]=0.999
                    else:
    ...
    if len(selectedWords)!=0:
        ...
        test[['EmotionSum','happyEmotions','SadEmotions','depressedEmotions','anxietyEmotions','stressEmotions','entropy']]=pd.DataFrame(test.EmotionSum.to_list(),index=test.index)
        test=test.iloc[:,-7:]
        testSample=pd.concat([pd.DataFrame(vector_representation.reshape(1,-1),columns=flatDF_weight.columns),test],axis=1)
...
while(True):
    text=input("Enter text you want  to analyze")
